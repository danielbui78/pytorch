...
inline const api::ShaderInfo& select_sum_buffer_shader(
    const api::ScalarType dtype) {
  return (dtype == api::kHalf) ? VK_KERNEL(sum_dim_buffer_f16)
                               : VK_KERNEL(sum_dim_buffer);
}

constexpr uint32_t kTwoPassBlockSize = 1024u;

Tensor run_sum_buffer_reduction_two_pass(
  ...
  check_storage_buffer_limit(v_input, "sum input");
  check_storage_buffer_limit(v_pass1, "sum pass1 output");
  check_storage_buffer_limit(v_output_buffer, "sum output");
  ...
  context->submit_compute_job(
      VK_KERNEL(sum_dim_buffer_pass1),
      pipeline_barrier,
  ...
  context->submit_compute_job(
      VK_KERNEL(sum_dim_buffer_pass2),
      pipeline_barrier,
...
Tensor run_sum_buffer_reduction(
    api::Context* const context,
    vTensor& v_input,
    const std::vector<int64_t>& output_size,
    const std::vector<int64_t>& full_output_size,
  ...
  context->submit_compute_job(
      // shader descriptor
      select_sum_buffer_shader(output_dtype),
      // pipeline barrier
      pipeline_barrier,
...
Tensor sum_dim(
  ...
  if (should_use_fp16_guard(use_buffer_path, output_dtype)) {
    ...
    Tensor reduced_float =
        sum_dim(input_float, buffer_dim, keepdim, c10::ScalarType::Float);
  ...
  if (should_use_two_pass(use_buffer_path, output_dtype, dim_size)) {
    ...
    return run_sum_buffer_reduction_two_pass(
        context,
        v_input,
        output_size,
        full_output_size,
  ...
  if (use_buffer_path) {
    ...
    return run_sum_buffer_reduction(
        context,
        v_input,
        output_size,
        full_output_size,
